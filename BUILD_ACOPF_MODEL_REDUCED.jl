    ...
    for lin = 1:nCIR
        if DCIR.l_status[lin] == 1 # Check if the branch is ON
            ...
            if any(!iszero, all_cap)
                ...
                # push!(s_ik_constraint, JuMP.@constraint(model, P_ik[lin]^2 + Q_ik[lin]^2 <= (all_cap[index_cap] / base_MVA)^2))
                # push!(s_ki_constraint, JuMP.@constraint(model, P_ki[lin]^2 + Q_ki[lin]^2 <= (all_cap[index_cap] / base_MVA)^2))
